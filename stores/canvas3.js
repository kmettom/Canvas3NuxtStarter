...
export const useCanvas3Store = defineStore("canvas3Store", {
  ...
  actions: {
    ...
    setTextMeshPositions() {
      ...
      for (var i = 0; i < this.textStore.length; i++) {
        this.textStore[i].mesh.position.x =
          this.textStore[i].htmlEl.getBoundingClientRect().left -
          this.width / 2;
    ...
    removeScrollActiveElement(elNode) {
      ...
      for (
        var i = 0;
        i < this.scroll.DOM.onScrollActivateElements.length;
        i++
      ) {
        if (
    ...
    async addImageAsMesh(
      ...
      if (
        meshUniforms.uAniInImage ||
        imgHtmlEl.dataset.activeScroll === "true"
      ) {
        this.activateMesh(meshId, true);
      }
